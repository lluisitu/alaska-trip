...
- **The phone calls now live in `tools/CALLS.md`**, generated by `build_calls.py` and ordered by
  arrival date rather than by stop. 27 items across 23 stops as of this writing, up from the three
  that used to be listed here — the trail, dog and offroad research produced a great many "the
  authority publishes nothing and this number would settle it" findings, and scattered across the
  db they were sentences nobody would ever find again. None is a blocker: every one is recorded on
  the dashboard as unknown rather than guessed, which is the safe state. The original three are
  still in there — Salida RV Resort's 20-year rule (719-882-1569), Yosemite Westlake 40 ft
  (209-878-3847), Wrangell View 32 ft vs 70 ft (907-823-2265).
